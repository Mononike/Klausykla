...
export default function App() {
  ...
  useEffect(() => {
      ...
      navigator.xr.isSessionSupported('immersive-vr').then(supported => {
        ...
        controller1.addEventListener('selectstart', () => {
          if(teleportMarker.material.opacity > 0.1){
            ...
            const offsetPos = teleportMarker.position;
            const transform = new XRRigidTransform(
              { x: offsetPos.x - camera.position.x, y: offsetPos.y + 1.6 - camera.position.y, z: offsetPos.z - camera.position.z, w: 1 }
            );
            camera.position.set(offsetPos.x, offsetPos.y + 1.6, offsetPos.z);
          }
        });

        // Thumbstick judejimas VR — skaitomas kiekviename kadre
        vrThumbstickMove = () => {
          const session = renderer.xr.getSession();
          if(!session) return;
          for(const source of session.inputSources){
            if(!source.gamepad) continue;
            const axes = source.gamepad.axes; // [0]=left/right, [1]=up/down, [2]=left/right, [3]=up/down
            const handedness = source.handedness;

            if(handedness === 'left' && axes.length >= 4){
              // Kairys thumbstick — horizontalus judejimas
              const lx = Math.abs(axes[2]) > 0.15 ? axes[2] : 0;
              const ly = Math.abs(axes[3]) > 0.15 ? axes[3] : 0;
              if(lx !== 0 || ly !== 0){
                // Judejimas kameros kryptimi
                ...
                camera.position.addScaledVector(dir, -ly * 0.08);
                camera.position.addScaledVector(right, lx * 0.08);
              }
            }

            if(handedness === 'right' && axes.length >= 4){
              // Desinis thumbstick — kilimas/leidimasis
              const ry = Math.abs(axes[3]) > 0.15 ? axes[3] : 0;
              if(ry !== 0){
                camera.position.y -= ry * 0.06; // aukstyn = neigiamas y axis
    ...
    for(let i=0;i<300;i++){const th=Math.random()*Math.PI*2,ph=Math.acos(Math.random()*2-1),r=100+Math.random()*250;sp[i*3]=r*Math.sin(ph)*Math.cos(th);sp[i*3+1]=r*Math.cos(ph);sp[i*3+2]=r*Math.sin(ph)*Math.sin(th);}
    ...
    SCULPTURES.forEach((s) => {
      ...
      // Point light
      const light = new THREE.PointLight(s.color, 0, 20);
      light.position.set(x, s.y + 1.5, z);
      scene.add(light);

      mesh.userData.light = light;
      mesh.userData.beam = beam;

      // Garsas sukuriamas TIK kai zaidejas arteja (lazy loading)
      sculptureData.push({ mesh, sound: null, soundType: s.sound, x, z, y: s.y, color: s.color, emissive: s.emissive, soundActive: false });
    });
    ...
    renderer.setAnimationLoop(() => {
      ...
      // VR teleport ray
      if(isVR && controller1){
        // Thumbstick judejimas
        if(vrThumbstickMove) vrThumbstickMove();
      ...
      sculptureData.forEach((sd, i) => {
        ...
        // LAZY GARSAS — sukuriamas tik kai arteja, istrinamas kai nutolsta
        if(d < 15 && !sd.soundActive){
          // Sukuriam garsa
          try {
            sd.sound = createSpatialSound(sd.soundType);
            sd.soundActive = true;
          } catch(e){ console.warn('Sound create failed:', e); }
        } else if(d >= 20 && sd.soundActive){
          // Per toli — istrinam garsa (taupo resursus)
          try { if(sd.sound) sd.sound.dispose(); } catch(e){}
          sd.sound = null;
          sd.soundActive = false;
        }

        // Atnaujinti garsa jei aktyvus
        if(sd.sound && sd.soundActive){
          if(sd.sound.panner){
    ...
    return () => {
      ...
      renderer.dispose();
      sculptureData.forEach(sd=>{try{if(sd.sound)sd.sound.dispose();}catch(e){}});
